...
async def handle_custom_rom(message: types.Message):
    ...
    if success: await status.edit_text("✅ Готово!")
    else: await status.edit_text("❌ Ошибка.")
...
async def show_my_library(callback: types.CallbackQuery):
    ...
    for i, name in enumerate(current_rom_list):
        kb.inline_keyboard.append([InlineKeyboardButton(text=f"🕹 {name[:20]}", callback_data=f"idx_{i}")])
    kb.inline_keyboard.append([InlineKeyboardButton(text="⬅️ Меню", callback_data="back")])
    await callback.message.edit_text("💾 Твои игры:", reply_markup=kb)

@dp.callback_query(F.data.startswith("idx_"))
async def rom_opts(callback: types.CallbackQuery):
    idx = int(callback.data.split("_")[1])
    name = current_rom_list[idx]
    url = f"{WEB_APP_URL}?rom={name}"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👤 Соло", web_app=WebAppInfo(url=url))],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="my_library")]
    ])
    await callback.message.edit_text(f"🎮 {name}", reply_markup=kb)

@dp.callback_query(F.data == "back")
async def back(callback: types.CallbackQuery):
    await callback.message.edit_text("👾 Retro Co-Op Club", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📁 Моя полка", callback_data="my_library")]
    ]))
